...
export function usePropagator(
  ...
  /** t1 of the interval currently displayed (ms), null before first frame */
  ...
  /** Interval length in simulated seconds, scaled to the time warp. */
  const intervalFor = useCallback((speed: number) => {
    return Math.min(Math.max(speed * 1.5, 2), 240)
  }, [])
  ...
  useEffect(() => {
    ...
    worker.onmessage = (e: MessageEvent<PropagatorResponse>) => {
      ...
      if (msg.type === 'ready') {
        ...
        setDegraded(null)
        // request the first frame immediately
        const now = clock.getTime()
        const dur = intervalFor(speedRef.current) * 1000
        requestInterval(now, now + dur)
        return
      }
      if (msg.type === 'frame') {
        inFlightRef.current = false
        ...
        const now = clock.getTime()
        if (activeT1Ref.current === null || now >= frame.t0) {
  ...
  useEffect(() => {
    const id = setInterval(() => {
      ...
      const durMs = intervalFor(speedRef.current) * 1000
      const activeT1 = activeT1Ref.current
      const leadMs = Math.max(durMs * 0.35, speedRef.current * 600)
