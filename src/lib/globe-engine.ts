...
void main() {
  ...
  gl_FragColor = vec4(0.30, 0.55, 1.05, 1.0) * intensity * 2.0;
}
`
...
export class GlobeEngine {
  ...
  constructor(container: HTMLElement, cb: EngineCallbacks) {
    ...
    // Earth diameter ~70% of viewport height at this distance
    this.camera.position.set(1.15, -3.3, 1.6)
  ...
  private makeStars(): THREE.Points {
    ...
    for (let i = 0; i < N; i++) {
      ...
      z = (z / len) * r
      pos.set([x, y, z], i * 3)
      const b = 0.25 + Math.random() * 0.5 // restrained, stays under bloom threshold
  ...
  private applyViewOffset() {
    ...
    if (w >= 1024 && w > h) {
      this.camera.setViewOffset(w, h, -Math.round(w * 0.11), 0, w, h)
    } else {
  ...
  /** Lower the quality cap to DPR 1 if the device cannot keep up. */
